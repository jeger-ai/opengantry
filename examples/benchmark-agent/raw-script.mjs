 ...
 *
 * Vendored from examples/contrast-agent-script/agent-run.mjs for benchmark LOC parity.
 * Set BENCHMARK_ROOT to an isolated sandbox directory (with git init).
 */
...
const ROOT = process.env.BENCHMARK_ROOT ?? __dirname;
const STATE_FILE = path.join(ROOT, ".agent-state.json");
const TARGET = path.join(ROOT, "src", "lib", "greeting.js");
