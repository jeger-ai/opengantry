...
export const FORBIDDEN_SOURCE_EXTS = new Set([
  ".ts",
  ".tsx",
  ".jsx",
  ".cts",
...
export const PARSE_EXTS = new Set([".js", ".mjs"]);
  ...
  for (const ent of fs.readdirSync(scanRoot, { withFileTypes: true })) {
    ...
    if (hasSource) {
      findings.push({
        ...
        line: 1,
        message: `directory under workers/ has .js/.mjs but no package.json (not a worker): ${ent.name}`,
      });
    }
  }
  return findings;
...
export function checkForbiddenExtensions(scanRoot) {
  const findings = [];
  for (const file of walkFiles(scanRoot)) {
    const ext = path.extname(file);
    if (FORBIDDEN_SOURCE_EXTS.has(ext)) {
      findings.push({
        rule_id: "worker/js-only",
        file: path.relative(scanRoot, file),
        line: 1,
        message: `forbidden extension ${ext}; workers must be pure JavaScript (.js/.mjs only)`,
      });
    }
  }
  return findings;
