...
const ARCH_GATE = "node examples/iii-integration/scripts/run-iii-architecture.mjs";
...
function activationMarkdown({ configPath, gateLine, governedSnippet }) {
...
## 2. Cold lint (mission gate)

Bind worker-touching missions to:
...
function main() {
  ...
  console.log("\n2) Cold lint gate (bind to worker-touching missions / CI)");
  ...
  console.log(`   gate_command: ${gateLine}`);
  ...
  if (writeMd) {
    ...
    fs.writeFileSync(
      outPath,
      activationMarkdown({ configPath, gateLine, governedSnippet }),
    );
