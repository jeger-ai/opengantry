...
Ensure the OpenGantry worker is installed from the iii workers registry or your local checkout:
...
function bootstrap(targetRoot) {
  ...
  const gateLine = `node ${relFromCwd(path.join(EXAMPLE_ROOT, "scripts/run-iii-architecture.mjs"))}`;
...
function main() {
  ...
  console.log("   iii worker add opengantry   # or path to iii-hq/workers/opengantry");
  if (!cfg.exists) {
