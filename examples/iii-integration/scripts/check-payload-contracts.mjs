...
function isRegisterFunctionCall(node) {
  ...
  if (c.type === "MemberExpression" && !c.computed && c.property.type === "Identifier") {
    return c.property.name === "registerFunction";
  }
  if (c.type === "Identifier") return c.name === "registerFunction";
  return false;
}
...
export async function checkPayloadContracts(scanRoot) {
  ...
  for (const workerDir of listWorkerRoots(scanRoot)) {
    const workerRel = path.basename(workerDir);
    for (const file of walkFiles(workerDir)) {
      ...
      walk.simple(parsed.ast, {
        CallExpression(node) {
          ...
          const schemaName = schemaFileName(resolved);
          const schemaPath = path.join(workerDir, "schemas", schemaName);
          const relSchema = path.join(workerRel, "schemas", schemaName);
          if (!fs.existsSync(schemaPath)) {
            findings.push({
              rule_id: "payload/missing-schema",
              file: rel,
              line: node.loc?.start?.line ?? 1,
              message: `missing schema for ${resolved}: expected ${relSchema}`,
            });
            return;
          }
          validateSchemaFile(Ajv, schemaPath, relSchema, findings);
        },
      });
    }
  }
