...
- **Context injection:** `.cursor/rules/opengantry-gxt-substrate.mdc` (`alwaysApply: true`); `.cursor/hooks.json` — `sessionStart` mission scope + `beforeShellExecution` fallback guard; `.cursor/mcp.json` — `./scripts/mcp-launcher.sh`. The `mcp.json` payload is byte-identical for the specimen and adopters. There is no parity exemption for that file. Do not point Cursor at a raw `node` binary. The launcher `exec`s local `dist/cli` when present, else `gantry mcp serve`.
