...
- **Context injection:** `.cursor/rules/opengantry-gxt-substrate.mdc` (`alwaysApply: true`); `.cursor/hooks.json` — `sessionStart` mission scope + `beforeShellExecution` fallback guard; `.cursor/mcp.json` — `./scripts/mcp-launcher.sh` (`exec`s local `dist/cli` when present, else `gantry mcp serve`).
