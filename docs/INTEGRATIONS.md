...
- **Context injection:** Root [`AGENTS.md`](../AGENTS.md) (loaded automatically after `gantry init`). Optional [`.codex/config.toml`](https://developers.openai.com/codex/config-basic) for project defaults (sandbox, approval policy). Do not duplicate RULES/MANIFEST — link via `AGENTS.md`.
- **Session bootstrap (shell wrapper — no project hooks):**
