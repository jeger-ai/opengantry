...
- **Export vectors for Hub ingestion (spoke-owned)** — Hub consumes receipts via CI artifacts, PR attachments, `gantry attest --export` (detached `{ payload_b64, signature }` envelope per ADR-0036), or an optional future `gantry attest --git-note` (or equivalent). Export is an opt-in path out of the gitignored history dir; it does not embed proofs into every clone by default.
- **Org-scoped export config stays out of committed substrate** — `org_id`, HMAC pepper, and `pepper_version` for receipt v0.2.0+ live in `GANTRY_ORG_*` env vars or gitignored `.gitagent/foreman/ORG.export.local`. They MUST NOT be stored in `.gitagent/config.json` (whose digest is embedded in every receipt).
