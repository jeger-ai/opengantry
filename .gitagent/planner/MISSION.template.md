...
For each DoD row, the Verifier MUST map **PASS** to `EXECUTOR_LOG.md` only: a **verbatim quote** plus **line number or timestamp** from that file. Missing or non-matching trace → **Evidence Tampering** (auto-fail per RULES).
