...
1. **Single bootstrap command** — The executor obtains all scope and trace sinks from **`gapman runtime env`**.
...
3. **Trace sink** — The canonical worker-authored trace path is **`EXECUTOR_LOG.md`** at the repository root unless overridden at verify time (`gapman verify --executor-log`).
...
| `GXT_EXECUTOR_LOG` | Absolute path to **`EXECUTOR_LOG.md`** at repo root (same default as `gapman verify` without `--executor-log`). |
...
- The executor MUST append trace evidence required by **`gapman verify`** to the file referenced by **`GXT_EXECUTOR_LOG`** (or the path `--executor-log` if the verify step overrides it).
...
**`gapman verify`** performs git-proof Planner legislation, deterministic gate execution, and trace mapping (`--fuzzy-trace` for formatter line drift; `--break-glass` only with `GXT_BYPASS_SECRET`). **`gapman runtime env`** does **not** replace verify; it only standardizes inputs for the executor loop before or during execution.

When **`gapman runtime exec`** fails (forbidden zone, timeout, executor error), read **`GXT_LAST_ERROR_FILE`** for the machine-oriented `summary` and `remediation` fields — do not rely on exit code alone.

## Optional skill context stamps (advisory only)

Executors **may** append single-line reviewer context when an external IDE skill or tool assisted edits. These stamps are **not** authorization evidence and are **not** required for `gapman verify` PASS.
