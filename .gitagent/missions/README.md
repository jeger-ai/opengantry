...
Store mission files here (Markdown or YAML) so **`gapman verify`** can enforce **git-proof**: among the last **200** commits by default (override with **`gapman verify --scan-depth <n>`** or env **`GXT_MSN_SCAN_DEPTH`**), the newest `[MSN-XXXX]` commit authored by a Teacher email (see `GAPMAN_TEACHER_EMAILS` in the root [README](../../README.md#gapman-cli)) must modify the mission file being verified.
