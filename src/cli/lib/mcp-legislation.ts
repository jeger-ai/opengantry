...
export function handleDraftLegislation(input: DraftLegislationInput): DraftLegislationResult | { status: "error"; error: McpErrorBody } {
...
function findWrittenMissionPath(root: string, msnId: string): string | null {
...
export function handleExecuteLegislation(
  ...
  const missionAbs = findWrittenMissionPath(root, payload.msn_id);
  if (!missionAbs) {
    return mcpError("LEGISLATE_FAILED", "legislate completed but mission file not found", true);
  }

  const missionRel = formatRepoRelative(root, missionAbs);
  const commitMessage = `[${payload.msn_id}] Legislate ${payload.title.replace(/"/g, '\\"')}`;
  const commitTarget = missionRel;
  const suggestedHumanAction = `git add ${commitTarget} && git commit -m "${commitMessage}"`;
...
export function handleCheckSignature(
  ...
  const { root } = loadWorkspace();
  const missionAbs = path.isAbsolute(missionFilePath)
    ? path.resolve(missionFilePath)
    : path.join(root, missionFilePath.replace(/\\/g, path.sep));
...
export function handlePinMission(missionFilePath: string): Record<string, unknown> {
  ...
  const rel = formatRepoRelative(root, missionAbs);
  const pinPath = path.join(root, ".gitagent", "missions", ".active-mission");
...
export function handleResolveMission(explicit?: string): Record<string, unknown> {
  ...
  for (const c of candidates) {
    const abs = path.isAbsolute(c) ? c : path.join(root, c.replace(/\\/g, path.sep));
    if (fs.existsSync(abs)) {
      return {
        status: "resolved",
        mission_file_path: formatRepoRelative(root, abs),
      };
    }
  }
